...
public class HungerGame implements Comparable<HungerGame>, Runnable, Game {
	...
	public void loadFrom(ConfigurationSection section) {
		...
		if (section.contains("spawn-points")) {
			...
			for (String key : spawnPointsSection.getKeys(false)) {
				...
				Location loc = null;
				try {
					loc = HungerGamesBukkit.parseToLoc(str);
				}
		...
		if (section.contains("chests")) {
			...
			for (String key : chestsSection.getKeys(false)) {
				...
				float weight = 1f;
				try {
					loc = HungerGamesBukkit.parseToLoc(parts[0]);
		...
		if (section.contains("fixedchests")) {
			...
			for (String key : fixedChestsSection.getKeys(false)) {
				...
				Location loc = null;
				try {
					loc = HungerGamesBukkit.parseToLoc(split[0]);
				}
				catch (NumberFormatException e) {
				}
				if (loc == null) {
		...
		try {
			if (section.contains("spawn")) spawn = HungerGamesBukkit.parseToLoc(section.getString("spawn"));
		} 
		catch (NumberFormatException numberFormatException) {}
		HungerGamesBukkit.callEvent(new GameLoadEvent(this));
	}
	...
	public void saveTo(ConfigurationSection section) {
		...
		for (cntr = 0; cntr < spawnPoints.size(); cntr++) {
			...
			spawnPointsSection.set("spawnpoint" + (cntr + 1), HungerGamesBukkit.parseToString(loc));
		}
		cntr = 1;
		for (Location loc : chests.keySet()) {
			cntr++;
			chestsSection.set("chest" + cntr, HungerGamesBukkit.parseToString(loc) + "," + chests.get(loc));
		}
		
		cntr = 1;
		for (Location loc : fixedChests.keySet()) {
			fixedChestsSection.set("fixedchest" + cntr, HungerGamesBukkit.parseToString(loc) + "," + fixedChests.get(loc));
			cntr++;
		}
		...
		section.set("spawn", HungerGamesBukkit.parseToString(spawn));
		
		HungerGamesBukkit.callEvent(new GameSaveEvent(this));
	}

	public void run() {
		if (state != RUNNING) return;
		Random rand = HungerGamesBukkit.getRandom();
	...
	public boolean addReadyPlayer(LocalPlayer player) {
		if (state == DELETED) {
	...
	public void clearWaitingPlayers() {
		for (Iterator<String> it = stats.keySet().iterator(); it.hasNext();) {
			...
			LocalPlayer player = Bukkit.getPlayer(stat);
	...
	public boolean addSpectator(LocalPlayer player, LocalPlayer spectated) {
		...
		for (LocalPlayer p : getRemainingPlayers()) {
			p.hidePlayer(player);
		}
	...
	public void removeSpectator(LocalPlayer player) {
	...
	public String stopGame(boolean isFinished) {
		...
		if (state == PAUSED) { // Needed for inventory stuff
			for(String playerName : playerLocs.keySet()) {
				LocalPlayer p = Bukkit.getPlayer(playerName);
				if (p == null) continue;
		...
		for (LocalPlayer player : getRemainingPlayers()) {
			...
			teleportPlayerToSpawn(player);
			if (isFinished) HungerGamesBukkit.rewardPlayer(player);
		}
	...
	@Override
	public boolean startGame(LocalPlayer player, boolean immediate) {
	...
	public String startGame(int ticks) {
		...
		locTaskId = HungerGamesBukkit.scheduleTask(this, 20 * 120, 20 * 10); // Wait two minutes, then poll every 10 seconds
		spectatorSponsoringRunnable.setTaskId(HungerGamesBukkit.scheduleTask(spectatorSponsoringRunnable, 0, SpectatorSponsoringRunnable.pollEveryInTicks));
	...
	@Override
	public boolean pauseGame(LocalPlayer player) {
	...
	public String pauseGame() {
		...
		HungerGamesBukkit.callEvent(new GamePauseEvent(this));
		return null;
	}
	...
	public void addAndFillChest(Chest chest) {
		...
		if(!chests.keySet().contains(chest.getLocation()) && !blacklistedChests.contains(chest.getLocation())) {
			...
			HungerGamesBukkit.fillChest(chest, 0, itemsets);
	...
	public void fillInventories() {
	    ...
	    for (Location loc : chests.keySet()) {
		    ...
		    HungerGamesBukkit.fillChest(chest, chests.get(loc), itemsets);
	    }
	    for (Location loc : fixedChests.keySet()) {
		    if (prev != null && prev.getBlock().getFace(loc.getBlock()) != null) {
			    //Logging.debug("Cancelling a fill because previous was a chest");
		    ...
		    prev = loc;
		    Chest chest = (Chest) loc.getBlock().getState();
		    HungerGamesBukkit.fillFixedChest(chest, fixedChests.get(loc));   
	    }

	}

	@Override
	public synchronized boolean rejoin(LocalPlayer player) {
		if (state != RUNNING) {
			ChatUtils.error(player, Lang.getNotRunning(setup).replace("<game>", name));
			return false;
		}
	...
	@Override
	public synchronized boolean join(LocalPlayer player) {
	...
	private synchronized boolean playerEnteringPreCheck(LocalPlayer player) {
	    if (state == DELETED) {
		    ChatUtils.error(player, "That game does not exist anymore.");
		    return false;
	    }
	    ...
	    if (Config.getRequireInvClear(setup)) {
		    if(!HungerGamesBukkit.hasInventoryBeenCleared(player)) {
	 ...
	 * @return
	 */
	private synchronized boolean playerEntering(LocalPlayer player, boolean fromTemporary) {
	    Location loc;
	    if (!fromTemporary) {
	    ...
	    for (String string : spectators.keySet()) {
		    LocalPlayer spectator = Bukkit.getPlayer(string);
		    if (spectator == null) continue;
	...
	@Override
	public synchronized boolean leave(LocalPlayer player, boolean callEvent) {
		...
		if (callEvent) HungerGamesBukkit.callEvent(new PlayerLeaveGameEvent(this, player, PlayerLeaveGameEvent.Type.LEAVE));
		if (!Config.getAllowRejoin(setup)) {
	...
	@Override
	public synchronized boolean quit(LocalPlayer player, boolean callEvent) {
	    if (!contains(player)) {
		    ChatUtils.error(player, Lang.getNotInGame(setup).replace("<game>", name));
		    return false;
	    }
	    if (callEvent) HungerGamesBukkit.callEvent(new PlayerLeaveGameEvent(this, player, PlayerLeaveGameEvent.Type.QUIT));
	 ...
	 * @param player
	 */
	private synchronized void playerLeaving(LocalPlayer player, boolean temporary) {
		for (String string : spectators.keySet()) {
		    LocalPlayer spectator = Bukkit.getPlayer(string);
		    if (spectator == null) continue;
	...
	public boolean checkForGameOver(boolean notifyOfRemaining) {// TODO config option
		...
		List<LocalPlayer> remaining = getRemainingPlayers();
		if (remaining.size() < 2) {
			LocalPlayer winner = null;
	...
	public void killed(final LocalPlayer killer, final LocalPlayer killed, PlayerDeathEvent deathEvent) {
		...
		if (killedStat.getState() == PlayerState.DEAD) {
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(HungerGamesBukkit.getInstance(), new Runnable() {
				@Override
				public void run() {
			...
			if (Config.shouldRespawnAtSpawnPoint(setup)) {
				...
				Location respawn = randomLocs.get(HungerGamesBukkit.getRandom().nextInt(randomLocs.size()));
				killed.teleport(respawn, TeleportCause.PLUGIN);
			}
	...
	public List<LocalPlayer> getRemainingPlayers() {
	    List<LocalPlayer> remaining = new ArrayList<LocalPlayer>();
	    for (String playerName : stats.keySet()) {
		LocalPlayer player = Bukkit.getPlayer(playerName);
	...
	public boolean removeSpawnPoint(Location loc) {
		...
		while (iterator.hasNext()) {
			if (HungerGamesBukkit.equals(loc, l = iterator.next())) {
				iterator.remove();
				for (String playerName : spawnsTaken.keySet()) {
					Location comp = spawnsTaken.get(playerName);
					if (HungerGamesBukkit.equals(l, comp)) {
	...
	private static void dropInventory(LocalPlayer player) {
	...
	public void setEnabled(boolean flag) {
		...
		if (!flag) {
			...
			for (String s : stats.keySet()) {
				LocalPlayer p = Bukkit.getPlayer(s);
				if (p == null) continue;
	...
	private class PlayerComparator implements Comparator<LocalPlayer> {

		public PlayerComparator() {
		}

		public int compare(LocalPlayer p1, LocalPlayer p2) {
